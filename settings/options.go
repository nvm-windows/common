...
type Settings struct {
	...
	TrustedModules               []string `cfg:"trusted_modules" reg:"TrustedModules" help:"Trust firewall: modules allowed to auto-reshim after self-update. NOT/! negation; empty defaults to NOT ALL. May include an HTTPS policy URL; local entries are checked first and only untrusted modules are POSTed to the URL." hidden:"true"`
	...
	ApprovedModules              []string `cfg:"approved_modules" reg:"ApprovedModules" help:"Module firewall (certified): allow list for local installs. Empty defaults to ALL. May be a single HTTPS policy URL." hidden:"true"`
	ApprovedGlobalModules        []string `cfg:"approved_global_modules" reg:"ApprovedGlobalModules" help:"Module firewall (certified): allow list for global installs. Empty defaults to ALL. May be a single HTTPS policy URL." hidden:"true"`
	...
	ApplyVerboseFirewallMetadata bool     `cfg:"apply_verbose_firewall_metadata" reg:"ApplyVerboseFirewallMetadata" default:"false" help:"When true, firewall policy JWTs include verbose identity claims (idp_*) plus npm/pnpm profile/config when available." hidden:"true"`
	TrustedFirewallSigners       []string `cfg:"trusted_firewall_signers" reg:"TrustedFirewallSigners" help:"Optional TLS certificate organization allowlist for HTTPS firewall policy URLs (certified)." hidden:"true"`
	TrustedFirewallThumbprint    []string `cfg:"trusted_firewall_thumbprint" reg:"TrustedFirewallThumbprint" help:"Optional SHA-1 leaf thumbprint pins for HTTPS firewall policy URLs (certified)." hidden:"true"`
